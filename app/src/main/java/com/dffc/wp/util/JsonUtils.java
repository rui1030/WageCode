...
public class JsonUtils {
	...
	public static String toJsonDateSerializer(Object obj, final String dateformat) {
		Gson gson = new GsonBuilder().registerTypeHierarchyAdapter(Date.class, new JsonSerializer<Date>() {
			...
			public JsonElement serialize(Date src, Type typeOfSrc, JsonSerializationContext context) {
				return new JsonPrimitive(DateTimeUtils.format(src.getTime(), dateformat));
			}
	...
	public static <T> T toObjectDateSerializer(String json, Class<T> clazz, final String dateformat) {
		Gson gson = new GsonBuilder().registerTypeAdapter(Date.class, new JsonDeserializer<Date>() {
			...
			public Date deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
                ...
                try {
                    return DateTimeUtils.parse(date, dateformat);
	...
	/**
	 * retrofit 传递json参数时候 需要创建RequestBody进行传递
	 * @param jsonStr
	 * @return
     */
	public static RequestBody convert2RequestBody(String jsonStr){
		RequestBody body = RequestBody.create(RAW, jsonStr);
		return body;
	}
